...
import { ExamGenerator, SchemeGenerator } from './components/ExamSchemeTabs';
...
export default function App() {
  ...
  const handleGeneratePlan = async (inputs: PlanFormInputs, mode: 'AI' | 'Offline Engine') => {
    ...
    try {
      ...
      // The server silently falls back to the offline engine when Gemini
      // fails. Surface that clearly so the teacher is never surprised by
      // offline content in "AI" mode.
      if ((plan as any).generationMode !== 'AI') {
        const reason = (plan as any).aiError ? ` Reason: ${(plan as any).aiError}` : '';
        setErrorMsg(`AI engine is currently unavailable${reason}. This plan was generated by the high-precision Offline Engine instead.`);
      }
